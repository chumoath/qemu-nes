...
static void create_mmci(DeviceState *dev)
{
	DeviceState * mmc_dev;
	DriveInfo *dinfo;
	mmc_dev = sysbus_create_varargs("pl181", 0x090c0000, qdev_get_gpio_in(dev, 0x88), qdev_get_gpio_in(dev, 0x2a), NULL);
	dinfo = drive_get(IF_SD, 0, 0);
	if (dinfo) {
		DeviceState *card;
		card = qdev_new(TYPE_SD_CARD);
		qdev_prop_set_drive_err(card, "drive", blk_by_legacy_dinfo(dinfo), &error_fatal);
		qdev_realize_and_unref(card, qdev_get_child_bus(mmc_dev, "sd-bus"), &error_fatal);
	}
}


static void create_ethernet(DeviceState *dev)
{
	NICInfo *nd;
	nd = &nd_table[0];
	smc91c111_init(nd, 0x090d0000, qdev_get_gpio_in(dev, 52));
}
...
static void machvirt_init(MachineState *machine)
{
	...
	create_mmci(DEVICE(vms->gic));
	
	create_ethernet(DEVICE(vms->gic));
